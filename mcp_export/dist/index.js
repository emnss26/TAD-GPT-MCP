...
const ExportNwcShape = {
    ...
    exportLinks: z.boolean().optional(), // ⬅️ NUEVO: se mapea a NavisworksExportOptions.ExportLinks*
...
server.registerTool("export_nwc", {
    title: "Export NWC",
    description: "Exporta a Navisworks (.nwc). Acepta 1 o varias vistas por id/nombre. Si hay varias y no das filename, usa el nombre de la vista; si das filename, genera filename_<vista>.nwc.",
    inputSchema: ExportNwcShape,
}, async (rawArgs) => {
    const args = ExportNwcSchema.parse(rawArgs);
...
server.registerTool("export_dwg", {
    ...
    const args = ExportDwgSchema.parse(rawArgs);
...
server.registerTool("export_pdf", {
    ...
    const args = ExportPdfSchema.parse(rawArgs);
